...
const isImage = (type === 'imageMessage')
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
        ...
        ////DAFTAR BUTTON BY Putra
      const daftar1 = `Hai kak  ${pushname} ${ucapanWaktu} \n\nSebelum Menggunakan Putra Gans Verify Terlebih Dahulu Ya `
      ...
      // GAME 
        ...
        if (game.isfam(from, family100)) {
          ...
          if (tebakata.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
			...
			tc = `MAAF KA NOMER KAMU BLOM TERDAFTAR DI DATABASE SILAHKAN KETIK !verify UNTUK MENDAFTAR DI Putra Gans`
			ind = {
			...
			/////
let Putraads = await getBuffer(`https://telegra.ph/file/bb17024f1356f5fcd0d64.jpg`)
fakelink = (tekslink) => { 
	return {"externalAdReply": { "title": tekslink, "thumbnail": Putraads, "sourceUrl": "https://wa.me/qr/UCVMNO2VLMDBK1" }}
}
// AFK
	if (isGroup) {
...
Putra.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `ZBOTZ`
		        var grousp = await Putra.groupMetadata(from)
		       ...
		       Putra.sendMessage(from, options, text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        Putra.sendMessage(from, 'Berhasil Mengirim Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
case 'bugcombine':
if (!isOwner && !kyy.key.fromMe) return reply(mess.only.owner)
                 Putra.toggleDisappearingMessages(from, 'HACKED')
Putra.sendMessage(from, `zbotz`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !kyy.key.fromMe) return reply(mess.only.owner)
Putra.updatePresence(from, Presence.composing)
		       Putra.sendMessage(from, {degreesLatitude: 34.0184,
...
if (!isOwner && !kyy.key.fromMe) return reply(mess.only.owner)
Putra.updatePresence(from, Presence.composing)
Putra.sendMessage(from, 'yaudah oke',text, {
 quoted: {
  key: {
...
if (args.length < 1) return reply('Jumlahnya?')
for (let i = 0; i < args[0]; i++) {
Putra.sendMessage(from, `zbotz`, MessageType.extendedText,{
 quoted: {
key: {
...
`
                    ...
                    Putra.sendMessage(from, { contentText: `${menu}`, footerText: 'Putra Gans', buttons: [{ buttonId: `!command`, buttonText: { displayText: '📙MENU' }, type: 1 },{ buttonId: `!sewabot`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: Putraads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
case 'f':
case 'lock':
 Putramemek = {
...
Putra.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": fakeimage}, MessageType.location, {quoted: Putramemek})
           break
   case 'kalender':
              reply(`
🐣Day ${week} ${weton} 
...
🔖 ${prefix}wallgravity text1 text2
 `
sendButMessage(from, wibu, `created Putra Gans`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
`
        ...
        case 'simi':
              ...
              buttonsMessage = {footerText:'Putra Gans', imageMessage: imageMsg,
...
`
 ...
 Putra Gans☕︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
`
 ...
 Putra Gans☕︎`, imageMessage: imageMsg,
   ...
   case 'command':
               list = []
               listmenu = [`groupmenu`,`photoxy`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`porno`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`Putragroup`,`sewabot`]
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated Putra ads`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
...
*「 Putra Gans 」*`
         Putraads = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `!menu`,buttonText:{displayText: `🏷️MENU`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(Putraads, "imageMessage", { thumbnail: Putraads, })).imageMessage
	    ...
	    // console.log(e)
            setTimeout( () => {
			Putra.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Terdaftar Di Putra Gans*`)
		}, 2000)
        break
case 'sfire':
{
...
┃⬡ Yt YUSUFZ
┗━━⬣
NOTE : BOT ON 24 JAM KARNA DI RUN MENGGUNAKAN RDP
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `OWNER`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT`},type:1}]
      ...
      case 'payment':
      list = await Putra.prepareMessageFromContent(from, {
    "listMessage": {
      "title": "PAYMENT",
      "description": "CLICK",
      "buttonText": "",
  ...
  Putra.relayWAMessage(list, {waitForAck: true})
...
//------------------< Owner >-------------------
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             Putra.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 Z-BOT NEWS 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             sendMess(_.jid, `*「 Putra Gans NEWS 」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
      ...
      case 'broadcast2':
             ...
             bc = await Putra.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             Putra.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 Putra Gans NEWS 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
Putra.sendMessage(_.jid, 
			{"contentText": `*「 Putra Gans NEWS 」*\n\n${body.slice(4)}`,
			"footerText": 'Putra Gans',
			"buttons": [
			{"buttonId": `#command`,
...
│
└──「 *${botName}* 」`
             Putra.sendMessage(from, await getBuffer(urlinfo), image, {quoted: freply, caption: thankslort })
             break
case 'media':
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              Putra.sendMessage('6285215319934@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
